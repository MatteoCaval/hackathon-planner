...
- [ ] voting systems: as you can see all of this is for a team to go somewhere in a retreat, we need to accomodate everyone's needs. we want to persist these votes in firebase togetehr with the trip
	- [ ] a way of impersonify a person, somewhere maybe in the top having a list of people a choose the one to impersonate (it's all based on trust) and if your name is missing from the list you can add it
	- [ ] once the person is chose, we want to be able to add/remove vote in every kind of option (flight, acoommodation, destination)
